...
Post.prototype.addPost = function(){
	...
	var postImage = imageUrlFor;// write function to get image
...
Post.prototype.getAllPostsByAllUsers = function(){
	var totalPost = DB.getPostCount();
	var i = 0;
	var allPostArray = new Array();
	for(i = 0; i<totalPost ; i++){
		var createPostId = "p"+1;
		var postObject = JSON.parse(localStorage.getItem(createPostId));
		allPostArray.push(postObject);
	}
	return allPostArray;
}

Post.prototype.DisplayAllPosts = function(){
	var count = DB.getPostCount();
	document.getElementById("displayPost").innerHTML = "";
	for(var i=1;i<count;i++){	
		console.log(count);
		var temp = "p"+i;
		var post = JSON.parse(localStorage.getItem(temp));
		...
		this.postAddDate = "";
		this.postStatus = "";
		console.log(post.id + post.catId+post.title);
		var imagesArrayload = post.postImage;
		var tmpimg = "<img src='" + imagesArrayload + "' width=\"200px\">";
		document.getElementById("displayPost").innerHTML += tmpimg; 
	}
}
